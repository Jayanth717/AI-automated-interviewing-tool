...
class Predict:
    ...
    class NLTKPreprocessor(BaseEstimator, TransformerMixin):
        ...
        def tokenize(self, document):
            ...
            return np.squeeze(tokenized_document)

        def vectorize(self, doc):
            save_path = os.path.join(os.path.dirname(__file__), '..', 'Models', 'padding.pickle')
    ...
    def run(self, X, model_name):
        ...
        # Load model from .h5 directly
        save_path = os.path.join(os.path.dirname(__file__), '..', 'Models')
        classifier = load_model(os.path.join(save_path, model_name + '.h5'))
